                ...
                if len(set["code"]) == 3:
                    ...
                    for card in prints["data"][set["code"]]["cards"]:
                        ...
                        if _name(card) in library:
                            ...
                            if (
                                set["releaseDate"]
                                > library[_name(card)][_name(card)]["date"]
                            ):
                                library[_name(card)] = {_name(card): _get(card, set)}
                        ...
                        if "//" in card["name"]:
                            ...
                            if (_name(card, face_a) and _name(card, face_b)) in library:
                                ...
                                try:
                                    library[_name(card, face_a)][_name(card, face_a)][
                                        "types"
                                    ] = list_types
                                except KeyError:
                                    pass
                                library[_name(card, face_b)][_name(card, face_b)][
                                    "types"
                                ] = list_types
                            # Adding Face A to library
                            if _name(card, face_a) not in library:
                                library[_name(card, face_a)] = {
                                    _name(card, face_a): _get(card, set, face_a)
                                }
                            # Adding Face B to library
                            if _name(card, face_b) not in library:
                                library[_name(card, face_b)] = {
                                    _name(card, face_b): _get(card, set, face_b)
                                }
